...
MainToolBar::~MainToolBar() {
	...
	if (tooltip_aid_chk) {
		tooltip_aid_chk->Unbind(wxEVT_CHECKBOX, &MainToolBar::OnTooltipCheckbox, this);
	}
	if (tooltip_uid_chk) {
		tooltip_uid_chk->Unbind(wxEVT_CHECKBOX, &MainToolBar::OnTooltipCheckbox, this);
	}
	if (tooltip_door_chk) {
		tooltip_door_chk->Unbind(wxEVT_CHECKBOX, &MainToolBar::OnTooltipCheckbox, this);
	}
	if (tooltip_dest_chk) {
		tooltip_dest_chk->Unbind(wxEVT_CHECKBOX, &MainToolBar::OnTooltipCheckbox, this);
	}
	if (tooltip_itemid_chk) {
		tooltip_itemid_chk->Unbind(wxEVT_CHECKBOX, &MainToolBar::OnTooltipCheckbox, this);
	}
	if (tooltip_text_chk) {
		tooltip_text_chk->Unbind(wxEVT_CHECKBOX, &MainToolBar::OnTooltipCheckbox, this);
	}
	if (tooltip_script_chk) {
		tooltip_script_chk->Unbind(wxEVT_CHECKBOX, &MainToolBar::OnTooltipCheckbox, this);
	}
	if (tooltip_house_chk) {
		tooltip_house_chk->Unbind(wxEVT_CHECKBOX, &MainToolBar::OnTooltipCheckbox, this);
	}
}

void MainToolBar::CreateTooltipQuickControls() {
	tooltip_enable_chk = nullptr;
	tooltip_aid_chk = nullptr;
	tooltip_uid_chk = nullptr;
	tooltip_door_chk = nullptr;
	tooltip_dest_chk = nullptr;
	tooltip_itemid_chk = nullptr;
	...
	auto addCheckbox = [&](int id, const wxString& label, const wxString& tip, wxCheckBox*& out) {
		...
		out->SetToolTip(tip);
		out->Bind(wxEVT_CHECKBOX, &MainToolBar::OnTooltipCheckbox, this);
		standard_toolbar->AddControl(out);
	};

	addCheckbox(TOOLBAR_TOOLTIP_ENABLE, "Tooltips", "Enable or disable map tooltips", tooltip_enable_chk);
	addCheckbox(TOOLBAR_TOOLTIP_AID, "AID", "Show action ID (aid) in tooltips", tooltip_aid_chk);
	addCheckbox(TOOLBAR_TOOLTIP_UID, "UID", "Show unique ID (uid) in tooltips", tooltip_uid_chk);
	addCheckbox(TOOLBAR_TOOLTIP_DOOR, "Door", "Show door ID in tooltips", tooltip_door_chk);
	addCheckbox(TOOLBAR_TOOLTIP_DEST, "Dest", "Show teleport destination in tooltips", tooltip_dest_chk);
	addCheckbox(TOOLBAR_TOOLTIP_ITEMID, "Item ID", "Show item ID in tooltips", tooltip_itemid_chk);
...
void MainToolBar::SyncTooltipQuickControls() {
	...
	const bool tooltipsOn = g_settings.getBoolean(Config::TOOLTIP_SHOW) && g_settings.getBoolean(Config::SHOW_TOOLTIPS);
	tooltip_enable_chk->SetValue(tooltipsOn);
	tooltip_aid_chk->SetValue(g_settings.getBoolean(Config::TOOLTIP_SHOW_AID));
	tooltip_uid_chk->SetValue(g_settings.getBoolean(Config::TOOLTIP_SHOW_UID));
	tooltip_door_chk->SetValue(g_settings.getBoolean(Config::TOOLTIP_SHOW_DOORID));
	tooltip_dest_chk->SetValue(g_settings.getBoolean(Config::TOOLTIP_SHOW_DESTINATION));
	tooltip_itemid_chk->SetValue(g_settings.getBoolean(Config::TOOLTIP_SHOW_ITEMID));
...
void MainToolBar::UpdateTooltipQuickControlStates() {
	...
	const bool tooltipsOn = g_settings.getBoolean(Config::TOOLTIP_SHOW) && g_settings.getBoolean(Config::SHOW_TOOLTIPS);
	tooltip_aid_chk->Enable(tooltipsOn);
	tooltip_uid_chk->Enable(tooltipsOn);
	tooltip_door_chk->Enable(tooltipsOn);
	tooltip_dest_chk->Enable(tooltipsOn);
	tooltip_itemid_chk->Enable(tooltipsOn);
...
void MainToolBar::OnTooltipCheckbox(wxCommandEvent& event) {
	...
	switch (event.GetId()) {
		case TOOLBAR_TOOLTIP_ENABLE:
			g_settings.setInteger(Config::TOOLTIP_SHOW, checked);
			g_settings.setInteger(Config::SHOW_TOOLTIPS, checked);
			UpdateTooltipQuickControlStates();
			g_gui.UpdateMenus();
			break;
		case TOOLBAR_TOOLTIP_AID:
			g_settings.setInteger(Config::TOOLTIP_SHOW_AID, checked);
			break;
		case TOOLBAR_TOOLTIP_UID:
			g_settings.setInteger(Config::TOOLTIP_SHOW_UID, checked);
			break;
		case TOOLBAR_TOOLTIP_DOOR:
			g_settings.setInteger(Config::TOOLTIP_SHOW_DOORID, checked);
			break;
		case TOOLBAR_TOOLTIP_DEST:
			g_settings.setInteger(Config::TOOLTIP_SHOW_DESTINATION, checked);
			break;
		case TOOLBAR_TOOLTIP_ITEMID:
			g_settings.setInteger(Config::TOOLTIP_SHOW_ITEMID, checked);
			break;
		case TOOLBAR_TOOLTIP_TEXT:
			g_settings.setInteger(Config::TOOLTIP_SHOW_TEXT, checked);
			break;
		case TOOLBAR_TOOLTIP_SCRIPT:
			g_settings.setInteger(Config::TOOLTIP_SHOW_HASSCRIPT, checked);
			break;
