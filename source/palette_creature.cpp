...
CreaturePalettePanel::CreaturePalettePanel(wxWindow* parent, wxWindowID id) :
	...
	selected_creature_brush(nullptr) {
	...
	creature_list->Connect(wxEVT_LEFT_DOWN, wxMouseEventHandler(CreaturePalettePanel::OnCreatureListMouseDown), nullptr, this);
...
void CreaturePalettePanel::SelectFirstBrush() {
	BrushVector visible = ComputeVisibleBrushes(search_field->GetValue());
	if (visible.empty()) {
		return;
	}

	SetSelectedCreatureBrush(visible.front());
	if (use_sprite_view) {
		if (use_seamless_view) {
			seamless_panel->SelectBrush(visible.front());
		} else {
			sprite_panel->SelectBrush(visible.front());
		}
	} else {
		SyncCreatureListSelection(visible.front());
	}
}

Brush* CreaturePalettePanel::GetSelectedBrush() const {
	if (selected_creature_brush) {
		return const_cast<Brush*>(selected_creature_brush);
	}
...
bool CreaturePalettePanel::SelectBrush(const Brush* whatbrush) {
	...
	for (BrushVector::const_iterator iter = all_creature_brushes.begin(); iter != all_creature_brushes.end(); ++iter) {
		if (*iter == whatbrush) {
			SetSelectedCreatureBrush(whatbrush);
			SyncCreatureListSelection(whatbrush);
			if (use_sprite_view) {
				if (use_seamless_view) {
					seamless_panel->SelectBrush(whatbrush);
				} else {
...
void CreaturePalettePanel::SelectTileset(size_t index) {
	...
	const Brush* previous_selection = selected_creature_brush;
	selected_creature_brush = nullptr;
	...
	PopulateAllCreatureBrushes(tsc);
	UpdateCreatureDisplay();

	const Brush* to_select = nullptr;
	if (previous_selection) {
		for (BrushVector::const_iterator iter = all_creature_brushes.begin(); iter != all_creature_brushes.end(); ++iter) {
			if (*iter == previous_selection) {
				to_select = previous_selection;
				break;
			}
		}
	}
	...
	if (!to_select) {
		BrushVector visible = ComputeVisibleBrushes(search_field->GetValue());
		if (!visible.empty()) {
			to_select = visible.front();
		}
	}

	if (to_select) {
		SetSelectedCreatureBrush(to_select);
		SyncCreatureListSelection(to_select);
		if (use_sprite_view) {
			if (use_seamless_view) {
				seamless_panel->SelectBrush(to_select);
			} else {
				sprite_panel->SelectBrush(to_select);
			}
		}
		SelectCreatureBrush();
	} else {
...
void CreaturePalettePanel::SelectCreature(size_t index) {
	if (use_sprite_view) {
		if (index < sprite_panel->creatures.size()) {
			...
			SetSelectedCreatureBrush(sprite_panel->GetSelectedBrush());
		} else if (use_seamless_view && index < seamless_panel->creatures.size()) {
			seamless_panel->SelectIndex(index);
			SetSelectedCreatureBrush(seamless_panel->GetSelectedBrush());
		}
	} else if (creature_list->GetCount() > 0 && index < creature_list->GetCount()) {
		creature_list->SetSelection(index);
		SetSelectedCreatureBrush(reinterpret_cast<const Brush*>(creature_list->GetClientData(index)));
	}
	...
	if (use_sprite_view) {
		for (size_t i = 0; i < sprite_panel->creatures.size(); ++i) {
			if (sprite_panel->creatures[i]->getName() == name) {
				...
				SetSelectedCreatureBrush(sprite_panel->creatures[i]);
				break;
			}
		}
		if (!selected_creature_brush && use_seamless_view) {
			for (size_t i = 0; i < seamless_panel->creatures.size(); ++i) {
				if (seamless_panel->creatures[i]->getName() == name) {
					seamless_panel->SelectIndex(i);
					SetSelectedCreatureBrush(seamless_panel->creatures[i]);
					break;
				}
			}
		}
		...
		if (creature_list->SetStringSelection(wxstr(name))) {
			SetSelectedCreatureBrush(reinterpret_cast<const Brush*>(creature_list->GetClientData(creature_list->GetSelection())));
		} else {
			creature_list->SetSelection(0);
			if (creature_list->GetSelection() != wxNOT_FOUND) {
				SetSelectedCreatureBrush(reinterpret_cast<const Brush*>(creature_list->GetClientData(creature_list->GetSelection())));
			}
		}
	}
...
void CreaturePalettePanel::SelectCreatureBrush() {
	bool has_selection = selected_creature_brush != nullptr;

	if (!has_selection) {
		if (use_sprite_view) {
			if (use_seamless_view) {
				has_selection = seamless_panel->GetSelectedBrush() != nullptr;
			} else {
				has_selection = sprite_panel->GetSelectedBrush() != nullptr;
			}
		} else {
			has_selection = creature_list->GetCount() > 0 && creature_list->GetSelection() != wxNOT_FOUND;
		}
	}
...
void CreaturePalettePanel::OnListBoxChange(wxCommandEvent& event) {
	if (event.GetSelection() != wxNOT_FOUND) {
		SetSelectedCreatureBrush(reinterpret_cast<const Brush*>(creature_list->GetClientData(event.GetSelection())));
	}
	ActivateSelectedCreatureBrush();
}
...
void CreaturePalettePanel::OnClickSearchButton(wxCommandEvent& event) {
    ...
    BrushVector visible = ComputeVisibleBrushes(searchText);
    if (visible.empty() || is_double_click) {
        ...
        if (dialog.ShowModal() == wxID_OK) {
            ...
            if (new_creature) {
                ...
                if (brush) {
                    ...
                    all_creature_brushes.push_back(brush);
                    UpdateCreatureDisplay();
                    SetSelectedCreatureBrush(brush);
                    SyncCreatureListSelection(brush);
                    if (use_sprite_view) {
                        if (use_seamless_view) {
                            seamless_panel->SelectBrush(brush);
                        } else {
                            sprite_panel->SelectBrush(brush);
                        }
                    } else {
...
void CreaturePalettePanel::PopulateAllCreatureBrushes(const TilesetCategory* tsc) {
	all_creature_brushes.clear();
	if (!tsc) {
		return;
	}
	...
	for (BrushVector::const_iterator iter = tsc->brushlist.begin(); iter != tsc->brushlist.end(); ++iter) {
		if ((*iter)->isCreature()) {
			all_creature_brushes.push_back(*iter);
		}
	}
}

BrushVector CreaturePalettePanel::ComputeVisibleBrushes(const wxString& search_text) const {
	BrushVector visible_brushes;
	...
	bool isAllCreaturesCategory = index != wxNOT_FOUND && tileset_choice->GetString(index) == "All Creatures";
	...
	for (BrushVector::const_iterator iter = all_creature_brushes.begin(); iter != all_creature_brushes.end(); ++iter) {
		...
		} else if (wxstr(baseCreatureName).Lower().Find(searchLower) != wxNOT_FOUND) {
			match = true;
		}

		if (match) {
			visible_brushes.push_back(*iter);
...
void CreaturePalettePanel::RebuildCreatureList(const BrushVector& brushes) {
	creature_list->Clear();

	for (BrushVector::const_iterator iter = brushes.begin(); iter != brushes.end(); ++iter) {
...
void CreaturePalettePanel::UpdateCreatureDisplay() {
	BrushVector visible = ComputeVisibleBrushes(search_field->GetValue());

	if (use_sprite_view) {
		if (use_seamless_view) {
			seamless_panel->LoadCreatures(visible);
		} else {
			sprite_panel->LoadCreatures(visible);
		}
	} else {
		RebuildCreatureList(visible);
	}
}

void CreaturePalettePanel::SyncCreatureListSelection(const Brush* brush) {
	if (use_sprite_view || !brush) {
		return;
	}

	for (size_t i = 0; i < creature_list->GetCount(); ++i) {
		if (reinterpret_cast<const Brush*>(creature_list->GetClientData(i)) == brush) {
...
void CreaturePalettePanel::SetSelectedCreatureBrush(const Brush* brush) {
	selected_creature_brush = brush;
}

void CreaturePalettePanel::ActivateSelectedCreatureBrush() {
	if (!selected_creature_brush) {
		return;
	}
	...
	Brush* brush = const_cast<Brush*>(selected_creature_brush);
...
void CreaturePalettePanel::OnCreatureListMouseDown(wxMouseEvent& event) {
	if (!use_sprite_view) {
		int hit = creature_list->HitTest(event.GetPosition());
		if (hit != wxNOT_FOUND && hit == creature_list->GetSelection()) {
			const Brush* brush = reinterpret_cast<const Brush*>(creature_list->GetClientData(hit));
			if (brush) {
				SetSelectedCreatureBrush(brush);
				ActivateSelectedCreatureBrush();
				return;
...
void CreaturePalettePanel::FilterCreatures(const wxString& search_text) {
	if (tileset_choice->GetCount() == 0 || all_creature_brushes.empty()) {
		return;
	}

	UpdateCreatureDisplay();

	BrushVector visible = ComputeVisibleBrushes(search_text);
	if (!visible.empty()) {
		bool selected_visible = false;
		if (selected_creature_brush) {
			for (BrushVector::const_iterator iter = visible.begin(); iter != visible.end(); ++iter) {
				if (*iter == selected_creature_brush) {
		...
		SyncCreatureListSelection(selected_creature_brush);
		if (use_sprite_view) {
			if (use_seamless_view) {
				seamless_panel->SelectBrush(selected_creature_brush);
			} else {
				sprite_panel->SelectBrush(selected_creature_brush);
			}
		}
		SelectCreatureBrush();
		creature_brush_button->Enable(true);
	} else {
		creature_brush_button->Enable(selected_creature_brush != nullptr);
	}
}
...
void CreaturePalettePanel::OnSpriteSelected(wxCommandEvent& event) {
	Brush* brush = nullptr;
	if (use_seamless_view) {
		brush = seamless_panel->GetSelectedBrush();
	} else {
		brush = sprite_panel->GetSelectedBrush();
	}
