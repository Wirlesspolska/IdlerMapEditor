...
struct PendingTile {
	...
	uint32_t house_id = 0;
	bool is_house_tile = false;
	std::vector<Item*> items;

	PendingTile() = default;
	PendingTile(const PendingTile&) = delete;
	PendingTile& operator=(const PendingTile&) = delete;
	PendingTile(PendingTile&&) = default;
	PendingTile& operator=(PendingTile&&) = default;
...
struct OtbmTileAreaJob {
	uint16_t base_x = 0;
	uint16_t base_y = 0;
	uint8_t base_z = 0;
	std::vector<OtbmNodeSnapshot> tiles;
};

void addWarning(wxArrayString& warnings, const char* format, ...) {
	char buffer[4096];
	va_list args;
...
bool parseOtbmTileNode(
	...
	if (!tileNode->getByte(tile_type)) {
		addWarning(warnings, "Invalid tile type");
		return false;
	}
	if (tile_type != OTBM_TILE && tile_type != OTBM_HOUSETILE) {
		addWarning(warnings, "Unknown type of tile node");
		return false;
	}

	uint8_t x_offset, y_offset;
	if (!tileNode->getU8(x_offset) || !tileNode->getU8(y_offset)) {
		addWarning(warnings, "Could not read position of tile");
		return false;
	}

	pending.pos = Position(base_x + x_offset, base_y + y_offset, base_z);
	pending.is_house_tile = tile_type == OTBM_HOUSETILE;

	if (pending.is_house_tile) {
		if (!tileNode->getU32(pending.house_id)) {
			addWarning(warnings, "House tile without house data, discarding tile");
			return false;
		}
		if (pending.house_id == 0) {
			addWarning(warnings, "Invalid house id from tile %d:%d:%d", pending.pos.x, pending.pos.y, pending.pos.z);
		}
	}
		...
		switch (attribute) {
			case OTBM_ATTR_TILE_FLAGS: {
				...
				if (!tileNode->getU32(flags)) {
					addWarning(warnings, "Invalid tile flags of tile on %d:%d:%d", pending.pos.x, pending.pos.y, pending.pos.z);
				}
				pending.map_flags = flags;
				if (flags & TILESTATE_ZONE_BRUSH) {
					uint16_t zoneId = 0;
					do {
						if (!tileNode->getU16(zoneId)) {
							addWarning(warnings, "Invalid zone id of tile on %d:%d:%d", pending.pos.x, pending.pos.y, pending.pos.z);
						}

						if (zoneId != 0) {
							pending.zone_ids.push_back(zoneId);
						}
					} while (zoneId != 0);
				}
				break;
			...
			case OTBM_ATTR_ITEM: {
				Item* item = Item::Create_OTBM(loader, tileNode);
				if (item == nullptr) {
					addWarning(warnings, "Invalid item at tile %d:%d:%d", pending.pos.x, pending.pos.y, pending.pos.z);
				} else {
			...
			default: {
				addWarning(warnings, "Unknown tile attribute at %d:%d:%d", pending.pos.x, pending.pos.y, pending.pos.z);
				break;
			}
		}
	}
	...
	for (BinaryNode* itemNode = tileNode->getChild(); itemNode != nullptr; itemNode = itemNode->advance()) {
		...
		if (!itemNode->getByte(item_type)) {
			addWarning(warnings, "Unknown item type %d:%d:%d", pending.pos.x, pending.pos.y, pending.pos.z);
			continue;
		}
		if (item_type == OTBM_ITEM) {
			Item* item = Item::Create_OTBM(loader, itemNode);
			if (item) {
				if (!item->unserializeItemNode_OTBM(loader, itemNode)) {
					addWarning(warnings, "Couldn't unserialize item attributes at %d:%d:%d", pending.pos.x, pending.pos.y, pending.pos.z);
				}
				pending.items.push_back(item);
			}
		} else {
			addWarning(warnings, "Unknown type of tile child node");
		}
	}
...
struct ParsedTileArea {
	std::vector<PendingTile> tiles;
	wxArrayString warnings;
};

ParsedTileArea parseTileAreaJob(IOMapOTBM& loader, const OtbmTileAreaJob& job) {
	ParsedTileArea result;
	result.tiles.reserve(job.tiles.size());

	for (const OtbmNodeSnapshot& tile_snapshot : job.tiles) {
		std::vector<uint8_t> buffer;
		appendOtbmNodeSnapshot(buffer, tile_snapshot);
		if (buffer.empty()) {
			continue;
		}

		MemoryNodeFileReadHandle reader(buffer.data(), buffer.size());
		BinaryNode* tileNode = reader.getRootNode();
		if (!tileNode) {
			continue;
		}

		PendingTile pending;
		if (parseOtbmTileNode(loader, tileNode, job.base_x, job.base_y, job.base_z, pending, result.warnings)) {
			result.tiles.push_back(std::move(pending));
		}
	}
...
void mergePendingTile(Map& map, PendingTile& pending, std::mutex& merge_mutex, wxArrayString& warnings) {
	std::lock_guard<std::mutex> lock(merge_mutex);

	if (map.getTile(pending.pos)) {
		addWarning(warnings, "Duplicate tile at %d:%d:%d, discarding duplicate", pending.pos.x, pending.pos.y, pending.pos.z);
		return;
	}

	Tile* tile = map.allocator(map.createTileL(pending.pos));
	House* house = nullptr;
	if (pending.is_house_tile) {
		if (pending.house_id) {
			house = map.houses.getHouse(pending.house_id);
			if (!house) {
				house = newd House(map);
				house->setID(pending.house_id);
				map.houses.addHouse(house);
			}
		}
	}

	tile->setMapFlags(pending.map_flags);
	for (uint16_t zoneId : pending.zone_ids) {
	...
	map.setTile(pending.pos.x, pending.pos.y, pending.pos.z, tile);
}

void loadTownsFromSnapshot(Map& map, const OtbmNodeSnapshot& snapshot, wxArrayString& warnings) {
	std::vector<uint8_t> buffer;
	appendOtbmNodeSnapshot(buffer, snapshot);
	if (buffer.empty()) {
		return;
	}

	MemoryNodeFileReadHandle reader(buffer.data(), buffer.size());
	BinaryNode* townNode = reader.getRootNode();
	if (!townNode) {
		return;
	}
	...
	if (!townNode->getByte(town_type)) {
		addWarning(warnings, "Invalid town type (1)");
		return;
	}
	if (town_type != OTBM_TOWN) {
		addWarning(warnings, "Invalid town type (2)");
		return;
	}

	uint32_t town_id;
	if (!townNode->getU32(town_id)) {
		addWarning(warnings, "Invalid town id");
		return;
	}

	Town* town = map.towns.getTown(town_id);
	if (town) {
		addWarning(warnings, "Duplicate town id %d, discarding duplicate", town_id);
		return;
	}
	...
	if (!townNode->getString(town_name)) {
		addWarning(warnings, "Invalid town name");
		return;
	}
	...
	if (!townNode->getU16(x) || !townNode->getU16(y) || !townNode->getU8(z)) {
		addWarning(warnings, "Invalid town temple position");
		return;
	}
	pos.x = x;
	pos.y = y;
...
void loadWaypointsFromSnapshot(Map& map, const OtbmNodeSnapshot& snapshot, wxArrayString& warnings) {
	std::vector<uint8_t> buffer;
	appendOtbmNodeSnapshot(buffer, snapshot);
	if (buffer.empty()) {
		return;
	...
	MemoryNodeFileReadHandle reader(buffer.data(), buffer.size());
	BinaryNode* waypointNode = reader.getRootNode();
	if (!waypointNode) {
		return;
	}
	...
	if (!waypointNode->getByte(waypoint_type)) {
		addWarning(warnings, "Invalid waypoint type (1)");
		return;
	}
	if (waypoint_type != OTBM_WAYPOINT) {
		addWarning(warnings, "Invalid waypoint type (2)");
		return;
	}

	Waypoint wp;
	if (!waypointNode->getString(wp.name)) {
		addWarning(warnings, "Invalid waypoint name");
		return;
	}
	...
	if (!waypointNode->getU16(x) || !waypointNode->getU16(y) || !waypointNode->getU8(z)) {
		addWarning(warnings, "Invalid waypoint position");
		return;
	}
	wp.pos.x = x;
	wp.pos.y = y;
...
bool loadTileAreaInline(
	...
	for (BinaryNode* tileNode = mapNode->getChild(); tileNode != nullptr; tileNode = tileNode->advance()) {
		...
		if (tile_type == OTBM_TILE || tile_type == OTBM_HOUSETILE) {
			...
			if (map.getTile(pos)) {
				addWarning(warnings, "Duplicate tile at %d:%d:%d, discarding duplicate", pos.x, pos.y, pos.z);
				continue;
			}
			...
			if (tile_type == OTBM_HOUSETILE) {
				...
				if (house_id) {
					...
					addWarning(warnings, "Invalid house id from tile %d:%d:%d", pos.x, pos.y, pos.z);
				}
			}
			...
			while (tileNode->getU8(attribute)) {
				switch (attribute) {
					case OTBM_ATTR_TILE_FLAGS: {
						uint32_t flags = 0;
						if (!tileNode->getU32(flags)) {
							addWarning(warnings, "Invalid tile flags of tile on %d:%d:%d", pos.x, pos.y, pos.z);
						}
						...
						if (flags & TILESTATE_ZONE_BRUSH) {
							...
							do {
								if (!tileNode->getU16(zoneId)) {
									addWarning(warnings, "Invalid zone id of tile on %d:%d:%d", pos.x, pos.y, pos.z);
								}
					...
					case OTBM_ATTR_ITEM: {
						Item* item = Item::Create_OTBM(loader, tileNode);
						if (item == nullptr) {
							addWarning(warnings, "Invalid item at tile %d:%d:%d", pos.x, pos.y, pos.z);
						}
						tile->addItem(item);
						break;
					}
					default: {
						addWarning(warnings, "Unknown tile attribute at %d:%d:%d", pos.x, pos.y, pos.z);
						break;
					}
				}
			}
			...
			for (BinaryNode* itemNode = tileNode->getChild(); itemNode != nullptr; itemNode = itemNode->advance()) {
				...
				uint8_t item_type;
				if (!itemNode->getByte(item_type)) {
					addWarning(warnings, "Unknown item type %d:%d:%d", pos.x, pos.y, pos.z);
					continue;
				}
				if (item_type == OTBM_ITEM) {
					item = Item::Create_OTBM(loader, itemNode);
					if (item) {
						if (!item->unserializeItemNode_OTBM(loader, itemNode)) {
							addWarning(warnings, "Couldn't unserialize item attributes at %d:%d:%d", pos.x, pos.y, pos.z);
						}
						tile->addItem(item);
					}
				} else {
...
bool IOMapOTBM::loadMap(Map& map, NodeFileReadHandle& f) {
	...
	int nodes_loaded = 0;
	const int threadcount = std::max(g_settings.getInteger(Config::WORKER_THREADS), 1);
	const bool use_parallel = threadcount > 1;

	std::vector<OtbmTileAreaJob> tile_area_jobs;
	std::vector<OtbmNodeSnapshot> deferred_towns;
	std::vector<OtbmNodeSnapshot> deferred_waypoints;
	...
	for (BinaryNode* mapNode = mapHeaderNode->getChild(); mapNode != nullptr; mapNode = mapNode->advance()) {
		...
		if (node_type == OTBM_TILE_AREA) {
			...
			if (use_parallel) {
				...
				job.base_z = base_z;
				for (BinaryNode* tileNode = mapNode->getChild(); tileNode != nullptr; tileNode = tileNode->advance()) {
					job.tiles.push_back(tileNode->snapshotSubtree());
				}
				tile_area_jobs.push_back(std::move(job));
			} else {
				loadTileAreaInline(*this, map, mapNode, base_x, base_y, base_z, warnings);
			}
		} else if (node_type == OTBM_TOWNS) {
			if (use_parallel) {
				for (BinaryNode* townNode = mapNode->getChild(); townNode != nullptr; townNode = townNode->advance()) {
					deferred_towns.push_back(townNode->snapshotSubtree());
				}
			} else {
				for (BinaryNode* townNode = mapNode->getChild(); townNode != nullptr; townNode = townNode->advance()) {
					Town* town = nullptr;
			...
			if (use_parallel) {
				for (BinaryNode* waypointNode = mapNode->getChild(); waypointNode != nullptr; waypointNode = waypointNode->advance()) {
					deferred_waypoints.push_back(waypointNode->snapshotSubtree());
				}
			} else {
				for (BinaryNode* waypointNode = mapNode->getChild(); waypointNode != nullptr; waypointNode = waypointNode->advance()) {
					uint8_t waypoint_type;
	...
	if (use_parallel && !tile_area_jobs.empty()) {
		g_gui.SetLoadDone(40, "Parsing map tiles...");

		std::vector<ParsedTileArea> parsed_areas(tile_area_jobs.size());
		std::atomic<size_t> next_job { 0 };

		auto worker = [&]() {
			while (true) {
				const size_t job_index = next_job.fetch_add(1);
				if (job_index >= tile_area_jobs.size()) {
					break;
				}
				parsed_areas[job_index] = parseTileAreaJob(*this, tile_area_jobs[job_index]);
				if (job_index % 15 == 0) {
					const int progress = 40 + static_cast<int>((job_index * 40.0) / tile_area_jobs.size());
		...
		const int workers = std::min(threadcount, static_cast<int>(tile_area_jobs.size()));
		...
		g_gui.SetLoadDone(80, "Merging map tiles...");
		std::mutex merge_mutex;
		wxArrayString merge_warnings;
		size_t areas_merged = 0;
		for (ParsedTileArea& parsed_area : parsed_areas) {
			for (size_t i = 0; i < parsed_area.warnings.size(); ++i) {
				warnings.push_back(parsed_area.warnings[i]);
			}
			for (PendingTile& pending : parsed_area.tiles) {
				mergePendingTile(map, pending, merge_mutex, merge_warnings);
			}
			++areas_merged;
			if (areas_merged % 15 == 0) {
				const int progress = 80 + static_cast<int>((areas_merged * 20.0) / parsed_areas.size());
				g_gui.SetLoadDone(progress);
			}
		}
		for (size_t i = 0; i < merge_warnings.size(); ++i) {
			warnings.push_back(merge_warnings[i]);
		}
	}

	for (const OtbmNodeSnapshot& town_snapshot : deferred_towns) {
		loadTownsFromSnapshot(map, town_snapshot, warnings);
	}
	for (const OtbmNodeSnapshot& waypoint_snapshot : deferred_waypoints) {
		loadWaypointsFromSnapshot(map, waypoint_snapshot, warnings);
	}
