...
void appendOtbmNodeSnapshot(std::vector<uint8_t>& out, const OtbmNodeSnapshot& snap) {
	out.push_back(NODE_START);
	for (unsigned char byte : snap.data) {
		if (byte == NODE_START || byte == NODE_END || byte == ESCAPE_CHAR) {
			out.push_back(ESCAPE_CHAR);
		}
		out.push_back(byte);
	}
	for (const OtbmNodeSnapshot& child : snap.children) {
		appendOtbmNodeSnapshot(out, child);
	}
	out.push_back(NODE_END);
}

OtbmNodeSnapshot BinaryNode::snapshotSubtree() {
	OtbmNodeSnapshot snap;
	snap.data = data;

	BinaryNode* node_child = getChild();
	while (node_child) {
		snap.children.push_back(node_child->snapshotSubtree());
		node_child = node_child->advance();
	}
	return snap;
}
